          ...
          <tbody>
            ...
            {clients.map((c) => {
              if (c.subscriptions.length === 0) {
                return (
                  <tr key={c.id} style={{ borderTop: "1px solid #e2e8f0" }}>
                    <td style={td}>
                      <Link href={`/clientes/${c.id}`} style={{ color: "#5c1a4a", fontWeight: 600 }}>
                        {c.fullName}
                      </Link>
                    </td>
                    ...
                    <td style={{ ...td, color: "#94a3b8" }} colSpan={3}>
                      Sin programa inscrito
                    </td>
                    <td style={td}></td>
                    <td style={{ ...td, textAlign: "right", whiteSpace: "nowrap" }}>
                      <Link href={`/clientes/${c.id}`} style={{ color: "#5c1a4a", fontWeight: 600, fontSize: "0.85rem" }}>
                        Ver
                      </Link>
                    </td>
                  </tr>
              ...
              return c.subscriptions.map((sub, idx) => {
                ...
                const isFirst = idx === 0;
                return (
                  <tr key={sub.id} style={{ borderTop: isFirst ? "1px solid #e2e8f0" : "1px dashed #e2e8f0" }}>
                    <td style={td}>
                      {isFirst ? (
                        <Link href={`/clientes/${c.id}`} style={{ color: "#5c1a4a", fontWeight: 600 }}>
                          {c.fullName}
                    ...
                    <td style={{ ...td, whiteSpace: "nowrap" }}>{isFirst ? formatAge(computeAge(c.birthDate)) : ""}</td>
                    <td style={td}>{isFirst ? c.guardianName : ""}</td>
                    <td style={td}>{isFirst ? c.phone : ""}</td>
                    <td style={td}>{PROGRAM_LABEL[sub.program]}</td>
                    <td style={td}>{PLAN_LABEL[sub.planType]}</td>
                    <td style={{ ...td, color: overdue ? "#dc2626" : "#3d0f30", fontWeight: overdue ? 700 : 400, whiteSpace: "nowrap" }}>
                      {sub.dueDate.toLocaleDateString("es-CO")}
                    </td>
                    <td style={td}>
                      <span
                        style={{
                    ...
                    <td style={{ ...td, textAlign: "right", whiteSpace: "nowrap" }}>
                      ...
                      {isFirst && (
                        <>
                          <Link href={`/clientes/${c.id}`} style={{ color: "#5c1a4a", fontWeight: 600, marginRight: 12, fontSize: "0.85rem" }}>
                            Ver
                          </Link>
